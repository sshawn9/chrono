...
// class, derived from ChSoaMobilizedBody, is templated by the number of degrees
...
/// - setJointPrecalculated: as needed, pre-calculates any required quantities that do not change at current state
...
class ChSoaMobilizedBodyT : public ChSoaMobilizedBody {
  ...
  protected:
    ChSoaMobilizedBodyT(std::shared_ptr<ChSoaMobilizedBody> parent,  ///< parent body
                        const ChSoaMassProperties& mpropsB,          ///< mobilized body mass properties
                        const ChFramed& X_PF,                        ///< parent to fixed frame transform
                        const ChFramed& X_BM,                        ///< body to moving frame transform
    ...
    ///
    /// The concrete mobilizer is first given the opportunity to calculate and cache any pre-calculated quantities that
    ...
    /// The last operation that this function performs is to call this same function for all the children of this body
    /// to ensure the base-to-tip traversal.
    virtual void orProcPosVelAccID(const ChVectorDynamic<>& y, const ChVectorDynamic<>& yd, const ChVectorDynamic<>& ydd) override;
    ...
    /// The first operation that this function performs is to call this same function for all children of the current
    /// body to ensure the tip-to-base traversal.
    virtual void irProcForcesID(const ChVectorDynamic<>& y, const ChVectorDynamic<>& yd, const ChVectorDynamic<>& ydd) override;
    ...
    /// Pre-calculate any required quantities that do not change at current state.
    /// The default implementation of setJointPrecalculated is no-op. A mobilizer should implement this method only if
    /// it can take advantage of pre-calculated quantities.
  ...
  private:
    ...
    const ChVectorN<double, dof> myU(double* yd) const { return Eigen::Map<ChVectorN<double, dof>>(&yd[m_uIdx], dof, 1); }
...
template <int dof>
inline void ChSoaMobilizedBodyT<dof>::orProcAccFD(const ChVectorDynamic<>& y, const ChVectorDynamic<>& yd, ChVectorDynamic<>& ydd) {
    if (m_locked) {
...
template <int dof>
inline void ChSoaMobilizedBodyT<dof>::orProcPosVelAccID(const ChVectorDynamic<>& y, const ChVectorDynamic<>& yd, const ChVectorDynamic<>& ydd) {
    setJointPrecalculated(y);
...
template <int dof>
inline void ChSoaMobilizedBodyT<dof>::irProcForcesID(const ChVectorDynamic<>& y, const ChVectorDynamic<>& yd, const ChVectorDynamic<>& ydd) {
    // Request the children to perform same operation
